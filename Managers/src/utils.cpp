...
bool utils::in(const string& origin, const string& dest){
	if (dest.find(origin) != dest.npos) return true;
	return false;
}

bool utils::in(const char* origin, const string& dest){
	if (dest.find(origin) != dest.npos) return true;
	return false;
}

bool utils::in(const string& origin, const char* dest){
	if (string(dest).find(origin) != string().npos) return true;
	return false;
}

bool utils::in(const char* origin, const char* dest){
	if (string(dest).find(origin) != string().npos) return true;
	return false;
}
